...
public class DriverWorker extends Thread {
    ...
    private static final Map<Device, IFptr> FPTRS = new HashMap<>();
    ...
    public static synchronized IFptr getFptr(Device device) {
        if (!FPTRS.containsKey(device) && device.isActive()) {
            FPTRS.put(device, new Fptr());
        } else if (FPTRS.containsKey(device) && !device.isActive()) {
            IFptr fptr = FPTRS.remove(device);
            if (fptr.isOpened()) {
                fptr.close();
            }
            return null;
    ...
    public static synchronized void removeFptr(Device device) {
        if (FPTRS.containsKey(device)) {
            IFptr fptr = FPTRS.get(device);
            if (fptr.isOpened()) {
                fptr.close();
            }
            FPTRS.remove(device);
